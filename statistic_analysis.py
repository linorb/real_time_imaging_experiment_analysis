...
def calculate_p_value_for_one_session(session_path, chosen_cells, number_of_active_rois, number_of_permutations):
    list_of_trial_dirs = os.listdir(session_path)

    all_session_events = []
    for trial_dir_name in list_of_trial_dirs:
        if trial_dir_name[:7] == 'results':
    ...
    rest_of_cells[chosen_cells] = 0
    rest_of_cells = rest_of_cells > 0
    not_chosen_events = all_session_events[rest_of_cells, :]
    ...
    for dir_name in list_of_sessions:
        if dir_name[:4] == '2017':
            ...
            [p_value, number_of_activations, number_of_original_activations] = \
                calculate_p_value_for_one_session(session_path, CHOSEN_CELLS, NUMBER_OF_ACTIVE_ROIS, NUMBER_OF_PERMUTATIONS)
